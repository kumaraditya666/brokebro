...
import { initCursor, readCursor } from "@/components/fx/cursor";
import { explodeForPhase } from "@/lib/fx/journey";
...
function Particles({ count }: { count: number }) {
  ...
  useFrame((_, dt) => {
    ...
    pts.rotation.y += dt * 0.014;
    const cur = readCursor();
    // cursor ground projection (approx): push nearby particles, burst on click
    const cx = cur.x * 9;
    const cz = 1.5 - cur.y * 5;
    const pos = pts.geometry.attributes.position as THREE.BufferAttribute;
    const arr = pos.array as Float32Array;
    const kick = cur.down ? 3.2 : 1;
    const step = Math.min(dt, 0.05);
    for (let i = 0; i < count; i++) {
      const ix = i * 3;
      const dx = arr[ix] - cx;
      const dz = arr[ix + 2] - cz;
      const d = Math.hypot(dx, dz);
      if (cur.inside && d < 2.4 && d > 1e-4) {
        const f = ((2.4 - d) / 2.4) * step * 6 * kick;
        vel.current[ix] += (dx / d) * f;
        vel.current[ix + 2] += (dz / d) * f;
      }
      vel.current[ix] += (base[ix] - arr[ix]) * step * 0.6;
      vel.current[ix + 1] += (base[ix + 1] - arr[ix + 1]) * step * 0.6;
      vel.current[ix + 2] += (base[ix + 2] - arr[ix + 2]) * step * 0.6;
      vel.current[ix] *= 0.94;
      vel.current[ix + 1] *= 0.94;
      vel.current[ix + 2] *= 0.94;
      arr[ix] += vel.current[ix];
      arr[ix + 1] += vel.current[ix + 1];
      arr[ix + 2] += vel.current[ix + 2];
    }
...
/** Cursor ground ring: a soft marker where the cursor meets HP. */
function CursorRing() {
  const ref = useRef<THREE.Mesh>(null);
  const p = useRef({ x: 0, z: 1.5 });
  useFrame((_, dt) => {
    const m = ref.current;
    if (!m || REDUCED()) return;
    const cur = readCursor();
    const tx = cur.inside ? cur.x * 9 : 0;
    const tz = cur.inside ? 1.5 - cur.y * 5 : 1.5;
    const k = 1 - Math.exp(-8 * Math.min(dt, 0.05));
    p.current.x += (tx - p.current.x) * k;
    p.current.z += (tz - p.current.z) * k;
    m.position.set(p.current.x, 0.03, p.current.z);
    const mat = m.material as THREE.MeshBasicMaterial;
    mat.opacity = cur.inside ? Math.min(0.55, 0.18 + cur.speed * 0.0006) : 0;
  });
  return (
    <mesh ref={ref} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.03, 1.5]}>
...
function HeroObject({
  ...
  explode: number;
  par: MutableRefObject<{ x: number; y: number }>;
}) {
  ...
  useFrame((_, dt) => {
    ...
    if (curG.current) {
      const prox = 1 - Math.min(1, Math.hypot(par.current.x, par.current.y) / 1.5);
      curG.current.scale.setScalar((0.62 + 0.38 * Math.min(1, t * 1.35)) * (1 + prox * 0.035));
    }
  });
  ...
  return (
    <group position={[0, 0, 1.2]}>
      ...
      {cone && (
        <group>
          {/* axis extends as the assembly separates */}
          <Line points={[[0, 0, 0], [0, 3.5 + explode * 1.3, 0]]} color="#fbbf24" lineWidth={2} />
          <Line points={[[0, 0.02, 0], [1.9, 0.02, 0.62]]} color="#f472b6" lineWidth={1.2} transparent opacity={0.8} />
          {/* base rim separates downward */}
          <Line
            points={Array.from({ length: 33 }, (_, i) => {
              const a = (i / 32) * Math.PI * 2;
              return [Math.cos(a) * 1.125, 0.03 - explode * 0.45, Math.sin(a) * 1.125] as [number, number, number];
            })}
            color="#7dd3fc"
            transparent
            opacity={0.65}
          ...
          />
          {/* apex marker rises */}
          <mesh position={[0, 3.15 + explode * 0.55, 0]}>
            <sphereGeometry args={[0.07, 12, 12]} />
            <meshBasicMaterial color="#fbbf24" />
          </mesh>
          <Tag position={[0, 3.65 + explode * 1.3, 0]} tone="amber">70 mm</Tag>
...
function CameraRig({ mode, par, travelRef }: { mode: CamMode; par: MutableRefObject<{ x: number; y: number }>; travelRef: MutableRefObject<number> }) {
  ...
  useFrame((_, dt) => {
    ...
    const dolly = 1 - 0.24 * travel;
    const dx = (base[0] + par.current.x * 1.1 * k) * dolly;
    const dy = (base[1] + par.current.y * 0.7 * k) * dolly + travel * 0.9;
    const dz = base[2] * dolly;
    ...
    target.set(0, 1.4 + travel * 0.3, 1.0);
...
export default function HeroStage({
  ...
  fill?: boolean;
  journeyPhase?: number;
  travelRef?: MutableRefObject<number>;
}) {
  ...
  const staticTravel = useRef(0);
  const fallbackTravel = travelRef ?? staticTravel;
  const jp = journeyPhase ?? -1;
  const showViews = projecting || jp >= 2;
  const explode = explodeForPhase(jp);
  ...
  return (
    <div
      ...
      className={
        fill
          ? "relative h-full w-full overflow-hidden bg-[#04060b]"
          : "relative h-[560px] w-full overflow-hidden rounded-3xl border border-white/10 bg-[#04060b] md:h-[620px]"
      }
      onPointerMove={(e) => {
        ...
        if (par.current && r) {
          ...
          const prox = 1 - Math.min(1, Math.hypot(par.current.x, par.current.y) / 1.4);
          wrap.current?.style.setProperty("--prox", prox.toFixed(3));
        }
    ...
    >
      ...
      >
        ...
        <CursorRing />
        <HeroObject shape={shape} prev={prev} trans={trans} setHover={(v) => setHover(v)} spin={spin} explode={explode} par={par} />
        <CameraRig mode={projecting ? "front" : cam} par={par} travelRef={travelRef ?? fallbackTravel} />
      </Canvas>
      ...
      <div className="pointer-events-none absolute left-4 top-4 hidden rounded-2xl border border-white/10 bg-[#070c16]/80 p-3.5 backdrop-blur md:block">
        ...
        <div className="mt-2 flex items-center gap-1.5">
          <span className="font-mono text-[9px] text-slate-500">PROX</span>
          <span className="h-1 flex-1 overflow-hidden rounded-full bg-white/10">
            <span className="block h-full rounded-full bg-cyan-300/70" style={{ width: "calc(var(--prox, 0) * 100%)" }} />
          </span>
        </div>
      ...
      {showViews && (
        <div className="absolute inset-y-4 right-4 flex w-[210px] flex-col gap-2">
          ...
          <div className="rounded-xl border border-cyan-300/25 bg-cyan-300/5 p-2 font-mono text-[9.5px] leading-relaxed text-cyan-100/80">
            ...
            {jp >= 3 && <span className="mt-1 block text-amber-200">Ø50 · H70 · 30°VP construct the sheet.</span>}
          </div>
        </div>
      )}
    </div>
