...
import { phaseForProgress } from "@/lib/fx/journey";
...
const PHASE_CAPTIONS = [
  { n: "01", t: "RAW OBJECT", d: "A cone, alive in coordinate space." },
  { n: "02", t: "SEPARATION", d: "Axis extends · rim drops · apex rises." },
  { n: "03", t: "PROJECTION", d: "Rays cast front, top and side views." },
  { n: "04", t: "DRAWING", d: "Dimensions construct the sheet." },
];
...
export default function LandingPage() {
  ...
  // scroll journey: progress through the sticky 3D world (travel via ref = zero renders)
  ...
  return (
    <div className="relative min-h-screen overflow-x-clip bg-[#05070d]">
      ...
      {/* sticky 3D journey: one world, four states */}
      <section ref={journeyRef} className="relative z-10 h-[280vh] md:h-[380vh]">
        <div className="sticky top-0 h-[100svh] overflow-hidden">
          <HeroStage fill journeyPhase={phase} travelRef={travelRef} />
          {/* headline overlay */}
          <div className="pointer-events-none absolute inset-x-0 top-0 mx-auto max-w-6xl px-6 pt-8 md:pt-12">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative max-w-3xl">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/25 bg-cyan-300/10 px-3 py-1 text-[12px] text-cyan-200 backdrop-blur">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-cyan-300" />
                SYSTEM // ENGINEERING GRAPHICS · HP · VP · XY
              </div>
              <h1 className="font-display mt-4 max-w-3xl text-4xl font-bold leading-[1.02] tracking-tight md:text-6xl">
              ...
              <p className="font-display mt-3 max-w-2xl text-lg font-semibold tracking-wide text-slate-200">
                Understand. Visualize. Project.
              </p>
            </motion.div>
          </div>
          {/* phase caption */}
          <div className="pointer-events-none absolute bottom-8 left-4 md:left-8">
            <div key={phase} className="rounded-2xl border border-white/10 bg-black/60 px-4 py-3 backdrop-blur">
              <div className="font-mono text-[10px] tracking-[0.3em] text-cyan-300">{PHASE_CAPTIONS[phase].n} / 04</div>
              <div className="font-display mt-0.5 text-lg font-bold tracking-tight">{PHASE_CAPTIONS[phase].t}</div>
              <div className="mt-0.5 max-w-[240px] text-[12px] text-slate-400">{PHASE_CAPTIONS[phase].d}</div>
            </div>
          </div>
          {/* progress rail */}
          <div className="absolute right-4 top-1/2 hidden -translate-y-1/2 flex-col gap-2 md:flex">
            {[0, 1, 2, 3].map((i) => (
              <span key={i} className={`h-8 w-1 rounded-full transition-colors ${i <= phase ? "bg-cyan-300" : "bg-white/15"}`} />
            ))}
          </div>
          {phase === 0 && (
            <div className="pointer-events-none absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce font-mono text-[10.5px] tracking-[0.3em] text-slate-500">
              SCROLL ↓
            </div>
          )}
        </div>
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 pb-20 pt-10">
        ...
        <div id="features" className="mt-12 grid scroll-mt-6 gap-3 md:grid-cols-3">
          {FEATURES.map((f) => (
            <TiltCard key={f.title} className="relative rounded-2xl">
              <motion.button
                onClick={() => goFeature(f.route, f.tab, f.patch)}
                whileHover={{ y: -3 }}
                whileTap={{ scale: 0.98 }}
                className="glass pressable block h-full w-full cursor-pointer rounded-2xl p-5 text-left transition hover:border-cyan-300/50 hover:bg-cyan-300/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-300"
              >
